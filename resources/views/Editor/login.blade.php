            ...
            <div class="d-none d-lg-flex col-lg-8 align-items-center p-5">
                <div class="w-100 d-lg-flex align-items-center justify-content-center px-5">
                    @if ($configData['theme'] === 'dark')
                        <img class="img-fluid" src="{{ asset(mix('images/pages/login-v2-dark.svg')) }}" alt="Login V2" />
                    @else
                        <img class="img-fluid" src="{{ asset(mix('images/pages/login-v2.svg')) }}" alt="Login V2" />
                    @endif
                </div>
            </div>
        </div>
            ...
            $("#frmLogin").on("submit", function(event) {
                ...
                $.ajax({
                    ...
                    success: function(response, textStatus, xhr) {
                        if (xhr.status == 200) {
                            window.location.href = "{{ route('editor.cp') }}";
                        } else {
                            $("#alertError").show();
